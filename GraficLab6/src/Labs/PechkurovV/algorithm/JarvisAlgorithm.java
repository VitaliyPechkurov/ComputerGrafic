package Labs.PechkurovV.algorithm;
...
public class JarvisAlgorithm {
    ...
    private static final double INF = 9999999;
    private static final double EPS = 0.00001;

    public JarvisAlgorithm(ArrayList<Point2D> points){
        this.points = points;
    }
    ...
    private static double calcLineSegmentLength(Point2D a, Point2D b){
    ...
    private static int findPointIndexWithBiggestAngle(ArrayList<Point2D> points, int prevPointIndex, int originPointIndex,
                                                      boolean[] used){
        int index = 0;
        double angle = 0.0;
        ...
        for (int i=0; i<points.size(); i++){
            ...
            double curAngle = 0.0;
            if (prevPointIndex < 0){
                curAngle = calcAngleBetweenTwoVectorsWithTheSameOrigin(points.get(originPointIndex),
            ...
            double curDistance = calcLineSegmentLength(points.get(originPointIndex), points.get(i));

            if (Math.abs(curAngle - angle) < EPS){ //перевірка на рівність
